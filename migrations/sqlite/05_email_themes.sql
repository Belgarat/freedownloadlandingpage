...
CREATE TABLE IF NOT EXISTS email_themes (
    ...
    is_custom INTEGER DEFAULT 0, -- BOOLEAN as INTEGER in SQLite
    created_by INTEGER NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (category_id) REFERENCES email_theme_categories(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS email_theme_properties (
    ...
    property_type TEXT DEFAULT 'css' CHECK (property_type IN ('css', 'html', 'json')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (theme_id) REFERENCES email_themes(id) ON DELETE CASCADE,
...
INSERT OR IGNORE INTO email_theme_categories (id, name, description) VALUES
...
INSERT OR IGNORE INTO email_themes (id, name, description, category_id, is_default, is_custom) VALUES
(1, 'Classic Business', 'Clean and professional business theme', 1, 1, 0),
(2, 'Modern Marketing', 'Contemporary marketing theme with vibrant colors', 2, 1, 0),
(3, 'Newsletter Pro', 'Professional newsletter layout', 3, 1, 0),
(4, 'Simple Transactional', 'Minimal transactional email theme', 4, 1, 0),
(5, 'Dark Mode', 'Dark theme for modern applications', 1, 0, 0);

-- Insert theme properties for Classic Business
INSERT OR IGNORE INTO email_theme_properties (theme_id, property_key, property_value, property_type) VALUES
